  ...
  const accessChat = async (recipientId) => {
    try {
      ...
      const { data } = await axios.post(
        ...
        { recipientId },
        {
          //en axios, la data va segunda y luego las opciones (headers y demas)
          headers: {
            Authorization: `Bearer ${user.token}`,
  ...
  return (
    <>
      <Box className="flex justify-between items-center bg-white w-full py-5 px-10 border-[5px]">
        <Tooltip
          label="Busca un usuario para chatear"
          hasArrow
        ...
        <Text className="text-2xl font-sans text-gray-600">E-chat-2.0</Text>
        ...
        <div>
          <Menu>
            ...
            <MenuList className="pl-2">
              {!notifications.length && "No hay mensajes nuevos"}
                ...
                notifications.map((notification) => (
                  <MenuItem key={notification._id} onClick={() => {
                    ...
                    {notification.chat.isGroupChat
                      ? `Nuevo mensaje en ${notification.chat.chatName}`
                      : `Nuevo mensaje de ${getSender(user, notification.chat.users)}`}
                  </MenuItem>
                ))}
            </MenuList>
          </Menu>
